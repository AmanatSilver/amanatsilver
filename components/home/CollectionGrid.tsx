  ...
  return (
    <section className="py-20 md:py-32 container mx-auto px-6 md:px-12">
      <div className="text-center mb-12 md:mb-16">
        ...
        <p className="text-3xl md:text-4xl lg:text-5xl font-light serif italic text-stone-800">Distinct Stories</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12">
        {collections.map((collection, index) => (
          <Link key={collection.id} to="/collections" className="group block">
            ...
            <div className="aspect-[3/4.5] overflow-hidden bg-stone-100 rounded-2xl flex items-center justify-center">
              <img src={collection.heroImage} alt={collection.name} className="w-full h-full object-contain" />
            </div>
          </Link>
        ))}
      </div>
